...
char *get_sensor_indicator_value_string(uint8_t sensor_id, char *format, get_sensor_indicator_value_fun fun)
{
    ...
    else
    {
        ...
        {
            snprintf(app_screen_msg, APP_SCREEN_MSG_BUFFER_SIZE - 1, "故障");
        }
        else
        {
...
app_screen_paged_message_node_t main_messages[] = {
    {"温度: ", .handler = &get_water_temperature, .visibility = &get_sensor_temperature_visibility},
    {"溶氧: ", .handler = &get_water_do, .visibility = &get_sensor_do_visibility},
    {"pH: ", .handler = &get_water_ph, .visibility = &get_sensor_ph_visibility},

    // {"叶绿素:", .handler = &get_water_chlorophyll, .visibility = &get_sensor_chlorophyll_visibility},
    // {"COD:", .handler = &get_water_cod, .visibility = &get_sensor_cod_visibility},
    // {"氨氮:", .handler = &get_water_nh3, .visibility = &get_sensor_nh3_visibility},
    {"  未配置传感器", .handler = NULL, .visibility = &no_sensors_visibility},
