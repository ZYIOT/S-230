...
static uint16_t RS485_L6_status_message_deserialize(uint8_t *bytes, rs485_l6_status_pt message)
{
    ...
    pos += 2;
    message->read_wait = read_uint16_t_BE(&bytes[pos]);
    pos += 2;
    message->read = read_uint16_t_BE(&bytes[pos]);
    pos += 2;
    message->charge = read_uint16_t_BE(&bytes[pos]);
    pos += 2;
    message->charge_wait = read_uint16_t_BE(&bytes[pos]);
    pos += 2;
    return pos;
}
...
static int _parse_value_error(uint8_t error_code, void *ret, void *args)
{
    ...
    if (l6_status->error_count < 0xFF00)
    {
        l6_status->error_count += 1;
    }
    return RS485_OK;
}
...
int RS485_L6_set_register(rs485_port_pt rs485, uint16_t release, uint16_t read_wait, uint16_t read, uint16_t charge, uint16_t charge_wait)
{
    ...
    write_uint16_t_BE(read_wait, &(buffer[2]));
    write_uint16_t_BE(read, &(buffer[4]));
    write_uint16_t_BE(charge, &(buffer[6]));
    write_uint16_t_BE(charge_wait, &(buffer[8]));
