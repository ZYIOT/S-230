...
static int _check_sensor_orp(rs485_sensor_pt rs485, rs485_sensor_driver_pt sd);
...
static rs485_sensor_action_t _action = {
    ...
    .check_sensor = _check_sensor_orp,
...
// 此处传感器的 PN 命名，需要讨论下
static char _pn_match_name[MAX_MATCH_TYPE_COUNT][RS485_SENSOR_MAX_PN_LEN] = {"ORP-62"};
const static char _manufacturer_match[MAX_MATCH_TYPE_COUNT] = {RS485_DRIVER_ORP_MANUFACTURER_CHEMINS};
const static char _model_match[MAX_MATCH_TYPE_COUNT] = {RS485_SENSOR_MODEL_ID_ORP620};
const static char _pn_name[MAX_MATCH_TYPE_COUNT][RS485_SENSOR_MAX_PN_LEN] = {"ORP-620"};
// *** end


#define SENSOR_ID   (RS485_DRIVER_SENSOR_ID_ORP)
static rs485_sensor_driver_t sensor_driver;
const static char _PN_default[RS485_SENSOR_MAX_PN_LEN] = {"ORP-620"};
...
const static uint8_t _SN[RS485_SENSOR_MAX_SN_LEN] = {(SENSOR_SN_CODE_ORP620>>8), SENSOR_SN_CODE_ORP620, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int _parse_value(uint8_t buffer[RS485_BUFFER_MAX_SIZE], size_t len, void *ret, void *args)
{
    ...
    {
        indicator->value1 = value / 100.0;
    }
    else if(3 == dotcnt)
    {
        indicator->value1 = value / 1000.0;
    }
    else
    {
...
static int _check_sensor_orp(rs485_sensor_pt rs485, rs485_sensor_driver_pt sd)
{
#ifndef SENSOR_ORP_TEST_DEBUG
    uint8_t buffer[2] = {0};
    int rc = RS485_read_register(rs485->port, rs485->id, ORP_ADDR_REG, ORP_ADDR_REG_LEN, buffer, RS485_DRIVER_copy_buffer, NULL, NULL);
    RS485_CHECK_RC(rc)
...
{
#ifndef SENSOR_ORP_TEST_DEBUG
    int rc = RS485_read_register(rs485->port, rs485->id, ORP_VAL_REG, ORP_VAL_REG_LEN, indicator, _parse_value, _parse_value_error, NULL);
    if (rc != RS485_OK)
    {
        indicator->status = rc;
        return rc;
