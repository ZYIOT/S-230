...
static int _check_sensor_ph(rs485_sensor_pt rs485, rs485_sensor_driver_pt sd);
...
// 此处传感器的 PN 命名，需要讨论下
static char _pn_match_name[MAX_MATCH_TYPE_COUNT][RS485_SENSOR_MAX_PN_LEN] = {"PH-26"};
const static char _manufacturer_match[MAX_MATCH_TYPE_COUNT] = {RS485_DRIVER_PH_MANUFACTURER_CHEMINS};
const static char _model_match[MAX_MATCH_TYPE_COUNT] = {RS485_SENSOR_MODEL_ID_PH260};
const static char _pn_name[MAX_MATCH_TYPE_COUNT][RS485_SENSOR_MAX_PN_LEN] = {"PH-260"};
// *** end


#define SENSOR_ID   (RS485_DRIVER_SENSOR_ID_PH)
static rs485_sensor_driver_t sensor_driver;
const static char _PN_default[RS485_SENSOR_MAX_PN_LEN] = {"PH-260"};
...
static int _check_sensor_ph(rs485_sensor_pt rs485, rs485_sensor_driver_pt sd)
{
    ...
    uint8_t buffer[2] = {0};
    int rc = RS485_read_register(rs485->port, rs485->id, PH_ADDR_REG, PH_ADDR_REG_LEN, buffer, RS485_DRIVER_copy_buffer, NULL, NULL);
    RS485_CHECK_RC(rc)
...
{
#ifndef SENSOR_PH_TEST_DEBUG
    int rc = RS485_read_register(rs485->port, rs485->id, PH_VAL_REG, PH_VAL_REG_LEN, indicator, _parse_value, _parse_value_error, NULL);
    if (rc != RS485_OK)
    {
        indicator->status = rc;
        return rc;
