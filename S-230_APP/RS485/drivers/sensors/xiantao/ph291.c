...
static int _check_sensor_ph291(rs485_sensor_pt rs485, rs485_sensor_driver_pt sd);
...
static rs485_sensor_action_t _action = {
    ...
    .check_sensor = _check_sensor_ph291,
...
// 此处传感器的 PN 命名，需要讨论下
const static char _pn_match_name[MAX_MATCH_TYPE_COUNT][RS485_SENSOR_MAX_PN_LEN] = {"PH-29"};
const static char _manufacturer_match[MAX_MATCH_TYPE_COUNT] = {RS485_DRIVER_PH_MANUFACTURER_XT};
const static char _model_match[MAX_MATCH_TYPE_COUNT] = {RS485_SENSOR_MODEL_ID_PH291};
const static char _pn_name[MAX_MATCH_TYPE_COUNT][RS485_SENSOR_MAX_PN_LEN] = {"PH-291"};
// *** end


#define SENSOR_ID   (RS485_DRIVER_SENSOR_ID_PH)
static rs485_sensor_driver_t sensor_driver;
const static char _PN_default[RS485_SENSOR_MAX_PN_LEN] = {"PH-291"};
...
const static uint8_t _SN[RS485_SENSOR_MAX_SN_LEN] = {(SENSOR_SN_CODE_PH291>>8), SENSOR_SN_CODE_PH291, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00};
...
{
    rs485_sensor_indicator_pt indicator = (rs485_sensor_indicator_pt)ret;
    indicator->value1 = read_uint16_t_BE(&buffer[4]) / 10.0;
    indicator->value2 = read_uint16_t_BE(&buffer[2]) / 100.0;
    indicator->value3 = read_uint16_t_BE(&buffer[0]) / 100.0;
    indicator->status = RS485_OK;
...
static int _check_valid_indicator(rs485_sensor_indicator_pt indicator)
{
    ...
    || (indicator->value2 < DEFAULT_PH_MIN_VALUE) 
    || (indicator->value2 > DEFAULT_PH_MAX_VALUE))
        return RS485_ERROR;
    return RS485_OK;
}
...
static int _check_sensor_ph291(rs485_sensor_pt rs485, rs485_sensor_driver_pt sd)
{
#ifndef SENSOR_PH_TEST_DEBUG
    uint8_t buffer[2] = {0};
    int rc = RS485_read_register(rs485->port, rs485->id, PH291_BAUDRATE_REG, PH291_BAUDRATE_REG_LEN, buffer, RS485_DRIVER_copy_buffer, NULL, NULL);
    RS485_CHECK_RC(rc)
    if (read_uint16_t_BE(&buffer[0]) != PH291_BAUDRATE_9600)
    {
        return RS485_ERROR;
    }
    rc = RS485_read_register(rs485->port, rs485->id, PH291_SENSOR_ID_REG, PH291_SENSOR_ID_REG_LEN, buffer, RS485_DRIVER_copy_buffer, NULL, NULL);
    RS485_CHECK_RC(rc)
...
static int _read_value1(rs485_sensor_pt rs485, rs485_sensor_indicator_pt indicator)
{
    int rc = RS485_read_register(rs485->port, rs485->id, PH291_VALUE_REG, PH291_VALUE_REG_LEN, indicator, _parse_value, _parse_value_error, NULL);
    if (rc != RS485_OK)
    {
        indicator->status = rc;
        return rc;
