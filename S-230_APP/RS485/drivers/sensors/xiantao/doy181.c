...
static int _check_sensor_doy181(rs485_sensor_pt rs485, rs485_sensor_driver_pt sd);
...
static rs485_sensor_action_t _action = {
    ...
    .check_sensor = _check_sensor_doy181,
...
const static char _pn_match_name[MAX_MATCH_TYPE_COUNT][RS485_SENSOR_MAX_PN_LEN] = {"DO-Y18"};
const static char _manufacturer_match[MAX_MATCH_TYPE_COUNT] = {RS485_DRIVER_DO_MANUFACTURER_XT};
const static char _model_match[MAX_MATCH_TYPE_COUNT] = {RS485_SENSOR_MODEL_ID_DOY181};
const static char _pn_name[MAX_MATCH_TYPE_COUNT][RS485_SENSOR_MAX_PN_LEN] = {"DO-Y181"};
// *** end

#define SENSOR_ID   (RS485_DRIVER_SENSOR_ID_DO)
static rs485_sensor_driver_t sensor_driver;
const static char _PN_default[9] = {"DO-Y181"};
static char _PN[9];
...
const static uint8_t _SN[RS485_SENSOR_MAX_SN_LEN] = {(SENSOR_SN_CODE_DOY181>>8), SENSOR_SN_CODE_DOY181, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int _init_sensor_driver(rs485_sensor_driver_pt sd, uint8_t match_sensor_index)
{
    ...
    _action.pn_match_name = (char *)_pn_match_name[match_sensor_index];
...
static int _check_valid_indicator(rs485_sensor_indicator_pt indicator)
{
    if (
        ...
        indicator->value3 > SATURATION_MAX_VALUE ||
...
{
    uint8_t buffer[2] = {0};
    int rc = RS485_read_register(rs485->port, rs485->id, DOY181_BAUDRATE_REG, DOY181_BAUDRATE_REG_LEN, buffer, RS485_DRIVER_copy_buffer, NULL, NULL);
    RS485_CHECK_RC(rc)
    if (read_uint16_t_BE(&buffer[0]) != DOY181_BAUDRATE_9600)
    {
        return RS485_ERROR;
    }
    rc = RS485_read_register(rs485->port, rs485->id, DOY181_SENSOR_ID_REG, DOY181_SENSOR_ID_REG_LEN, buffer, RS485_DRIVER_copy_buffer, NULL, NULL);
    RS485_CHECK_RC(rc)
...
static int _read_compensation(rs485_sensor_s_t *rs485, uint8_t indicator_id, float *value)
{
    ...
    uint8_t buffer[2] = {0};
    int rc = RS485_read_register(rs485->port, rs485->id, DOY181_COMPENSATION_REG, DOY181_COMPENSATION_REG_LEN, buffer, RS485_DRIVER_copy_buffer, NULL, NULL);
...
static int _write_compensation(rs485_sensor_s_t *rs485, uint8_t indicator_id, float value)
{
    ...
    return RS485_write_register(rs485->port, rs485->id, DOY181_COMPENSATION_REG, f, NULL, NULL, NULL, NULL);
}
...
static int _write_calibrate(rs485_sensor_pt rs485, uint8_t type, void *params)
{
    ...
    int rc = RS485_write_register(rs485->port, rs485->id, DOY181_CALIBRATE_REG, type, NULL, NULL, NULL, NULL);
    RS485_CHECK_RC(rc)
    return RS485_OK;
}
...
{
    rs485_sensor_indicator_pt indicator = (rs485_sensor_indicator_pt)ret;
    indicator->value3 = read_float_BE(&buffer[0]);
    indicator->value2 = read_float_BE(&buffer[4]);
...
static int _read_value1(rs485_sensor_pt rs485, rs485_sensor_indicator_pt indicator)
{
    int rc = RS485_read_register(rs485->port, rs485->id, DOY181_SATURATION_VALUE_REG, (DOY181_SATURATION_REG_LEN + DOY181_DO_REG_LEN), indicator, _parse_value, _parse_value_error, NULL);
    if (rc != RS485_OK)
    {
        indicator->status = rc;
        return rc;
...
static int _match_pn_name(rs485_sensor_pt rs485, void *params)
{
    const char (*pn_name)[RS485_SENSOR_MAX_PN_LEN] = _pn_match_name;
...
#ifdef RS485_ENABLE_COMPATIBLE_MODE
    _action_compatible.pn_match_name = (char *)_pn_match_name;
    _action_compatible.PN = _PN_default;
    static rs485_sensor_driver_t sd_compatible = {.manufacturer = RS485_DRIVER_DO_MANUFACTURER_XT, .model = RS485_SENSOR_MODEL_ID_DOY181, .protocol = 0, .id = RS485_DRIVER_SENSOR_ID_DO, .action = &_action_compatible};
