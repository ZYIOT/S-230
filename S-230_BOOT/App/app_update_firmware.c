...
{
	uint32_t flag = OLD_FILE;
	char *download_pn_name = (char *)(FLASH_DOWNLOAD_START_ADDR + 0x400);
	char *app_pn_name = (char *)(FLASH_APP_START_ADDR + 0x400);
	uint8_t findSubstrFlag = 0;
	uint8_t i = 0;

	APP_LOG_trace("APP PN_CODE:%s\r\n", app_pn_name);
	*ret = UPDATE_NO_ERROR;
	if(download_firmware_info.download_len != download_firmware_info.firmware_info.file_len)
	{// download 尚未完成
		*ret = UPDATE_ERROR_DOWNLOAD_UNFINISH;
	...
	else
	{
		...
		{
			if(NULL != strstr(download_pn_name, board_pn_key_table[i]))
			{
				findSubstrFlag = 1;
			}
		}
